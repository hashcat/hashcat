...
int nvml_init (NVML_PTR *nvml)
{
  ...
  if (!nvml->lib)
  {
    ...
    if (RegOpenKeyExA (HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\NVIDIA Corporation\\Global\\NVSMI"), 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS)
    {
      if (RegQueryValueEx (hKey, TEXT("NVSMIPATH"), NULL, &Type, (LPBYTE) Buffer, &BufferSize) == ERROR_SUCCESS)
      {
        Buffer[BufferSize] = 0;
      }
      else
