...
#define NEXT_RPTOI(rule, rp, up) if (((up) = conv_ctoi((rule)[(rp)])) == -1) return RULE_RC_SYNTAX_ERROR;

void MANGLE_TOGGLE_AT(char *arr, int pos);
inline void MANGLE_TOGGLE_AT(char *arr, int pos) {
  if (class_alpha(arr[pos]))
    arr[pos] ^= 0x20;
}

void MANGLE_LOWER_AT(char *arr, int pos);
inline void MANGLE_LOWER_AT(char *arr, int pos) {
  if (class_upper(arr[pos]))
    arr[pos] ^= 0x20;
}


void MANGLE_UPPER_AT(char *arr, int pos);
inline void MANGLE_UPPER_AT(char *arr, int pos) {
...
void MANGLE_SWITCH(char *arr,int l, int r);
inline void MANGLE_SWITCH(char *arr,int l, int r){
  char c = arr[r];
  arr[r] = arr[l];
  arr[l] = c;
}
