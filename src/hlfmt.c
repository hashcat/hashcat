...
static int hlfmt_detect_passwd (MAYBE_UNUSED hashcat_ctx_t * hashcat_ctx, const char *line_buf, size_t line_len)
{
  ...
  if ((sep_cnt == 6) && ((sep5_first == '/') || (sep6_first == '/')))
    return 1;
...
const char *strhlfmt (const u32 hashfile_format)
{
  ...
  {
  case HLFMT_HASHCAT:
    return HLFMT_TEXT_HASHCAT;
  case HLFMT_PWDUMP:
    return HLFMT_TEXT_PWDUMP;
  case HLFMT_PASSWD:
    return HLFMT_TEXT_PASSWD;
  case HLFMT_SHADOW:
    return HLFMT_TEXT_SHADOW;
  case HLFMT_DCC:
    return HLFMT_TEXT_DCC;
  case HLFMT_DCC2:
    return HLFMT_TEXT_DCC2;
  case HLFMT_NETNTLM1:
    return HLFMT_TEXT_NETNTLM1;
  case HLFMT_NETNTLM2:
    return HLFMT_TEXT_NETNTLM2;
  case HLFMT_NSLDAP:
    return HLFMT_TEXT_NSLDAP;
  case HLFMT_NSLDAPS:
    return HLFMT_TEXT_NSLDAPS;
  }
...
void hlfmt_hash (hashcat_ctx_t * hashcat_ctx, u32 hashfile_format, char *line_buf, size_t line_len, char **hashbuf_pos, size_t * hashbuf_len)
{
  ...
  {
  case HLFMT_HASHCAT:
    hlfmt_hash_hashcat (hashcat_ctx, line_buf, line_len, hashbuf_pos, hashbuf_len);
    break;
  case HLFMT_PWDUMP:
    hlfmt_hash_pwdump (hashcat_ctx, line_buf, line_len, hashbuf_pos, hashbuf_len);
    break;
  case HLFMT_PASSWD:
    hlfmt_hash_passwd (hashcat_ctx, line_buf, line_len, hashbuf_pos, hashbuf_len);
    break;
  case HLFMT_SHADOW:
    hlfmt_hash_shadow (hashcat_ctx, line_buf, line_len, hashbuf_pos, hashbuf_len);
    break;
  }
}
...
void hlfmt_user (hashcat_ctx_t * hashcat_ctx, u32 hashfile_format, char *line_buf, size_t line_len, char **userbuf_pos, size_t * userbuf_len)
{
  ...
  {
  case HLFMT_HASHCAT:
    hlfmt_user_hashcat (hashcat_ctx, line_buf, line_len, userbuf_pos, userbuf_len);
    break;
  case HLFMT_PWDUMP:
    hlfmt_user_pwdump (hashcat_ctx, line_buf, line_len, userbuf_pos, userbuf_len);
    break;
  case HLFMT_PASSWD:
    hlfmt_user_passwd (hashcat_ctx, line_buf, line_len, userbuf_pos, userbuf_len);
    break;
  case HLFMT_SHADOW:
    hlfmt_user_shadow (hashcat_ctx, line_buf, line_len, userbuf_pos, userbuf_len);
    break;
  }
}
...
u32 hlfmt_detect (hashcat_ctx_t * hashcat_ctx, FILE * fp, u32 max_check)
{
  ...
  if (hashconfig->hash_mode == 5300)
    return HLFMT_HASHCAT;
  if (hashconfig->hash_mode == 5400)
    return HLFMT_HASHCAT;
  ...
  while (!feof (fp))
  {
    ...
    if (hlfmt_detect_pwdump (hashcat_ctx, line_buf, line_len))
      formats_cnt[HLFMT_PWDUMP]++;
    if (hlfmt_detect_passwd (hashcat_ctx, line_buf, line_len))
      formats_cnt[HLFMT_PASSWD]++;
    if (hlfmt_detect_shadow (hashcat_ctx, line_buf, line_len))
      formats_cnt[HLFMT_SHADOW]++;
