...
static int sort_by_tuning_db_entry (const void *v1, const void *v2)
{
  ...
  const int res2 = t1->attack_mode - t2->attack_mode;

  if (res2 != 0)
    return (res2);

  const int res3 = t1->hash_type - t2->hash_type;
...
int tuning_db_init (hashcat_ctx_t * hashcat_ctx)
{
  ...
  while (!feof (fp))
  {
    ...
    {
      if ((token_ptr[1][0] != '0') && (token_ptr[1][0] != '1') && (token_ptr[1][0] != '3') && (token_ptr[1][0] != '*'))
      {
      ...
      if ((token_ptr[3][0] != '1') && (token_ptr[3][0] != '2') && (token_ptr[3][0] != '4') && (token_ptr[3][0] != '8') && (token_ptr[3][0] != 'N'))
      {
      ...
      if (token_ptr[1][0] != '*')
        attack_mode = (int) strtol (token_ptr[1], NULL, 10);
      if (token_ptr[2][0] != '*')
        hash_type = (int) strtol (token_ptr[2], NULL, 10);
      if (token_ptr[3][0] != 'N')
        vector_width = (int) strtol (token_ptr[3], NULL, 10);
...
tuning_db_entry_t *tuning_db_search (hashcat_ctx_t * hashcat_ctx, const char *device_name, const cl_device_type device_type, int attack_mode, const int hash_type)
{
  ...
  {
    if (device_name_nospace[i] == ' ')
      device_name_nospace[i] = '_';
  }
