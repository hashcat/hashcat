...
static const char *ST_HASH        = "pbkdf2_sha256$10000$1135411628$bFYX62rfJobJ07VwrUMXfuffLfj2RDM2G6/BrTrUWkE=";
...
int module_hash_encode (MAYBE_UNUSED const hashconfig_t *hashconfig, MAYBE_UNUSED const void *digest_buf, MAYBE_UNUSED const salt_t *salt, MAYBE_UNUSED const void *esalt_buf, MAYBE_UNUSED const void *hook_salt_buf, MAYBE_UNUSED const hashinfo_t *hash_info, char *line_buf, MAYBE_UNUSED const int line_size)
{
  ...
  base64_encode (int_to_base64, (const u8 *) tmp, 32, (u8 *) tmp_buf);

  // output

  const int line_len = snprintf (line_buf, line_size, "%s$%u$%s$%s", SIGNATURE_DJANGOPBKDF2, salt->salt_iter + 1, (const char *) pbkdf2_sha256->salt_buf, tmp_buf);
