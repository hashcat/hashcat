...
static const u64   KERN_TYPE      = 8501;
...
static const char *ST_HASH        = "$racfp$*1313*E7D7E66D00018000000800320010001000000000000000000000000000000000CF57A0F7E9286EEB639717B60B8BF7A4";
...
static const char *SIGNATURE_RACF_KDFAES = "$racfp$";
...
int module_hash_decode (MAYBE_UNUSED const hashconfig_t *hashconfig, MAYBE_UNUSED void *digest_buf, MAYBE_UNUSED salt_t *salt, MAYBE_UNUSED void *esalt_buf, MAYBE_UNUSED void *hook_salt_buf, MAYBE_UNUSED hashinfo_t *hash_info, const char *line_buf, MAYBE_UNUSED const int line_len)
{
  ...
  if ((line_len < (7 + 1 + 1 + 1 + 16 + 1 + 8 + 24 + 32 + 16)) || (line_len > (7 + 1 + 8 + 1 + 16 + 1 + 8 + 24 + 32 + 16))) return (PARSER_GLOBAL_LENGTH);
  if (memcmp (SIGNATURE_RACF_KDFAES, line_buf, strlen(SIGNATURE_RACF_KDFAES))) return (PARSER_SIGNATURE_UNMATCHED);
  
  u32 *out_digest     = (u32 *) digest_buf;
  u8 *salt_buf_pc_ptr = (u8 *) salt->salt_buf_pc;
  
  u8 *username_pos = (u8 *)line_buf + strlen(SIGNATURE_RACF_KDFAES) + 1;
  u8 *hash_pos = (u8 *) strchr ((const char *) username_pos, '*');
  if (hash_pos == NULL) return (PARSER_SEPARATOR_UNMATCHED);
  u32 username_len = hash_pos - username_pos;
  if (username_len > 8) return (PARSER_SALT_LENGTH);
  if (username_len == UINT_MAX) return (PARSER_SALT_LENGTH);

  for (u32 i = 0; i < username_len; i++)
  {
    salt_buf_pc_ptr[i] = ascii_to_ebcdic[(int) username_pos[i]];
  }
  for (u32 i = username_len; i < 8; i++)
  {
    salt_buf_pc_ptr[i] = 0x40;
  }

  u32 hash_len = line_len - username_len - strlen(SIGNATURE_RACF_KDFAES) - 2;
  if (hash_len != 96) return (PARSER_HASH_LENGTH);
  ...
  for (u32 i = 0; i < 16; i++)
  {
      salt_buf_pc_ptr[i+8]  = hex_convert (hash_pos[(2*i)+1]) << 0;
  ...
  out_digest[0] = hex_to_u32 ((const u8 *) &hash_pos[ 0]);
  out_digest[1] = hex_to_u32 ((const u8 *) &hash_pos[ 8]);
