...
{
  if (((arr_len * times) + arr_len) >= RP_PASSWORD_SIZE)
    return (arr_len);
...
{
  if ((arr_len + arr2_cpy) > RP_PASSWORD_SIZE)
...
static int mangle_dupeblock_prepend (char arr[RP_PASSWORD_SIZE], int arr_len, int ulen)
{
  ...
  if ((arr_len + ulen) >= RP_PASSWORD_SIZE)
    return (arr_len);
...
static int mangle_dupechar (char arr[RP_PASSWORD_SIZE], int arr_len)
{
  if (arr_len == 0)
    return (arr_len);
  if ((arr_len + arr_len) >= RP_PASSWORD_SIZE)
    return (arr_len);
...
int _old_apply_rule (const char *rule, int rule_len, char in[RP_PASSWORD_SIZE], int in_len, char out[RP_PASSWORD_SIZE])
{
  ...
  if (in_len < 0 || in_len > RP_PASSWORD_SIZE)
    return (RULE_RC_REJECT_ERROR);
  ...
  for (rule_pos = 0; rule_pos < rule_len; rule_pos++)
  {
    ...
    case RULE_OP_MANGLE_LREST_UFIRST:
      out_len = mangle_lrest (out, out_len);
      if (out_len)
        MANGLE_UPPER_AT (out, 0);
      break;

    case RULE_OP_MANGLE_UREST_LFIRST:
      out_len = mangle_urest (out, out_len);
      if (out_len)
        MANGLE_LOWER_AT (out, 0);
      break;
    ...
    case RULE_OP_MANGLE_SWITCH_FIRST:
      if (out_len >= 2)
        mangle_switch_at (out, out_len, 0, 1);
      break;

    case RULE_OP_MANGLE_SWITCH_LAST:
      if (out_len >= 2)
        mangle_switch_at (out, out_len, out_len - 1, out_len - 2);
      break;
    ...
    case RULE_OP_MANGLE_REPLACE_NP1:
      ...
      if ((upos >= 0) && ((upos + 1) < out_len))
        mangle_overstrike (out, out_len, upos, out[upos + 1]);
      break;
    ...
    case RULE_OP_MANGLE_EXTRACT_MEMORY:
      ...
      NEXT_RULEPOS (rule_pos);
      NEXT_RPTOI (rule, rule_pos, upos2);
      if ((out_len = mangle_insert_multi (out, out_len, upos2, mem, mem_len, upos, ulen)) < 1)
        return (out_len);
      break;
    ...
    case RULE_OP_MANGLE_APPEND_MEMORY:
      if (mem_len < 1)
        return (RULE_RC_REJECT_ERROR);
      if ((out_len + mem_len) >= RP_PASSWORD_SIZE)
        return (RULE_RC_REJECT_ERROR);
    ...
    case RULE_OP_MANGLE_PREPEND_MEMORY:
      if (mem_len < 1)
        return (RULE_RC_REJECT_ERROR);
      if ((mem_len + out_len) >= RP_PASSWORD_SIZE)
        return (RULE_RC_REJECT_ERROR);
    ...
    case RULE_OP_REJECT_CONTAIN:
      NEXT_RULEPOS (rule_pos);
      if (strchr (out, rule[rule_pos]) != NULL)
        return (RULE_RC_REJECT_ERROR);
      break;
    ...
    case RULE_OP_REJECT_EQUAL_FIRST:
      NEXT_RULEPOS (rule_pos);
      if (out[0] != rule[rule_pos])
        return (RULE_RC_REJECT_ERROR);
      break;

    case RULE_OP_REJECT_EQUAL_LAST:
      NEXT_RULEPOS (rule_pos);
      if (out[out_len - 1] != rule[rule_pos])
        return (RULE_RC_REJECT_ERROR);
      break;
...
int run_rule_engine (const int rule_len, const char *rule_buf)
{
  ...
  if (rule_len == 1)
    if (rule_buf[0] == RULE_OP_MANGLE_NOOP)
      return 0;
