...
// Reading wordlists: a list of sources laid end to end into one keyspace, the seek database that makes
// a position in it cheap to reach, and the line by line read itself. Every feed that takes a wordlist
// wants all of it, so it is a file a feed includes rather than a plugin of its own, the same way
// seekdb.c is.
//
// Nothing here reaches into a plugin's own context. A feed hands over its feed_global_t and its
// feed_thread_t, which lets a feed keep them inside something larger, as the table feed keeps its
// table beside them.
//
...
static int wordlist_next_source (generic_global_ctx_t *global_ctx, feed_global_t *feed_global, generic_thread_ctx_t *thread_ctx, feed_thread_t *feed_thread, u8 *out_buf, const int out_size);

static feed_thread_t *wordlist_thread_init (generic_thread_ctx_t *thread_ctx);
static void           wordlist_thread_term (feed_thread_t *feed_thread);
...
static bool source_add (generic_global_ctx_t *global_ctx, feed_global_t *feed_global, const char *path)
{
  struct stat s;
...
static bool source_add_path (generic_global_ctx_t *global_ctx, feed_global_t *feed_global, const char *path)
{
  if (hc_path_is_directory (path) == false)
  {
    const bool rc = source_add (global_ctx, feed_global, path);
...
static bool source_seek_frame (feed_thread_t *feed_thread, const u64 *entry)
{
  ...
  else
  {
    ...
    // does not, and --cache-path exists so that a cluster shares exactly that directory. Reading
...
static bool source_open (generic_thread_ctx_t *thread_ctx, feed_thread_t *feed_thread, feed_global_t *feed_global, const u64 idx)
{
...
// Building the source list out of a range of the feed's arguments. The range is the caller's because
// a feed that takes something else on the command line as well, as the table feed takes its table,
// still wants every wordlist argument laid end to end the same way.

static bool wordlist_init (generic_global_ctx_t *global_ctx, feed_global_t *feed_global, const int from, const int to)
{
  ...
  {
    if (source_add_path (global_ctx, feed_global, global_ctx->workv[i]) == false) return false;
  }
...
// The sources and what the status line was told about them. The feed_global_t itself belongs to
// whoever allocated it, which is not always a plugin's whole context.

static void wordlist_term (generic_global_ctx_t *global_ctx, feed_global_t *feed_global)
{
  ...
  feed_global->sources     = NULL;
  feed_global->sources_cnt = 0;
}

static u64 wordlist_keyspace (generic_global_ctx_t *global_ctx, generic_thread_ctx_t *thread_ctx, hashcat_ctx_t *hashcat_ctx, feed_global_t *feed_global)
{
  // The seek database is built by reading a file once, and this is the reader that does it. It is its
  // own rather than the first device's, because a feed that wraps this one keeps something else in
  // that device's context and there is no reader to borrow there.

  feed_thread_t *feed_thread = wordlist_thread_init (thread_ctx);

  if (feed_thread == NULL)
  {
    error_set (global_ctx, "%s", thread_ctx->error_msg);
  ...
  for (u64 i = 0; i < feed_global->sources_cnt; i++)
  {
    ...
    if (seekdb_file == NULL)
    {
      ...
      wordlist_thread_term (feed_thread);
    ...
    // The same fold kept where the core cannot mark it. See feed_global_t.

    feed_global->ident = paw64 (&source_ident, sizeof (source_ident), feed_global->ident);
    ...
    if (source_open (thread_ctx, feed_thread, feed_global, i) == false)
    {
      error_set (global_ctx, "%s", thread_ctx->error_msg);
...
static int wordlist_next_source (generic_global_ctx_t *global_ctx, feed_global_t *feed_global, generic_thread_ctx_t *thread_ctx, feed_thread_t *feed_thread, u8 *out_buf, const int out_size)
{
  ...
  {
    if (source_open (thread_ctx, feed_thread, feed_global, 0) == false) return GENERIC_RC_ERROR;
  }
  ...
  while (feed_thread->fd_off >= feed_thread->fd_len)
  {
    ...
    if (source_open (thread_ctx, feed_thread, feed_global, next_idx) == false) return GENERIC_RC_ERROR;
  }

  const int word_len = wordlist_next (global_ctx, feed_global, thread_ctx, feed_thread, out_buf, out_size);
...
static int wordlist_next (generic_global_ctx_t *global_ctx, feed_global_t *feed_global, generic_thread_ctx_t *thread_ctx, feed_thread_t *feed_thread, u8 *out_buf, const int out_size)
{
  ...
  if (fd_off >= fd_len)
  {
    const int word_len = wordlist_next_source (global_ctx, feed_global, thread_ctx, feed_thread, out_buf, out_size);
...
static bool wordlist_seek (feed_global_t *feed_global, generic_thread_ctx_t *thread_ctx, feed_thread_t *feed_thread, const u64 offset)
{
  ...
  if (source_open (thread_ctx, feed_thread, feed_global, idx) == false) return false;
