...
static const char *const USAGE_BIG_PRE_HASHMODES[] =
{
  ...
  "     --pcfg-ahf-terminals-min   | Num  | Skip AHF terminal types if count < NUM               | --pcfg-ahf-terminals-min=30",
  "     --pcfg-ahf-type            | Num  | PCFG AHF-type: see references below                  | --pcfg-ahf-type=1",
  "     --pcfg-burst-first         |      | Burst iterates on first token instead of last        |",
  "     --pcfg-burst-size          | Num  | Candidates generated before structure switch         | --pcfg-burst-size=50000",
  "     --pcfg-keyspace-max        | Num  | Skip structures with keyspace > NUM                  | --pcfg-keyspace-max=1000000",
  "     --pcfg-loopback            |      | After -a0, train PCFG model from cracks and run -a10 |",
  "     --pcfg-mode                | Num  | PCFG-mode, see references below.                     | --pcfg-mode=2",
  "     --pcfg-model               | File | Path to pre-trained PCFG model file                  | --pcfg-model=example.dict.pcfg",
  "     --pcfg-model-diff          | File | Compare with --pcfg-model and show differences       |",
  "     --pcfg-model-info          |      | Show model statistics and exit                       |",
  "     --pcfg-model-save          | File | Save trained/merged model to file                    | --pcfg-model-save=example.dict.pcfg",
  "     --pcfg-model-update        |      | Update existing model with new training data         |",
  "     --pcfg-models-merge        | File | Merge multiple models into one                       | --pcfg-models-merge=example.dict.1.pcfg --pcfg-models-merge=example.dict.2.pcfg",
  "     --pcfg-omen-keyspace-max   | Num  | Skip structures with keyspace > NUM to avoid stalls  | --pcfg-omen-keyspace-max=1000000000",
  "     --pcfg-omen-cost-max       | Num  | Set OMEN end cost                                    | --pcfg-omen-cost-max=10",
  "     --pcfg-omen-cost-min       | Num  | Set OMEN start cost                                  | --pcfg-omen-cost-min=2",
  "     --pcfg-omen-max-alloc-perc | Num  | Limit large buffer allocation to this percentage     | --pcfg-omen-max-alloc-perc=90",
  "     --pcfg-omen-stats          |      | Show detailed OMEN stats live and at session end     |",
  "     --pcfg-omen-type           | Num  | PCFG OMEN-type: see references below.                | --pcfg-omen-type=1",
  "     --pcfg-perf-threshold      | Str  | Attempt to optimize attack if rate < threshold       | --pcfg-perf-threshold loop:100:1h,cost:10:5m,struct:5:30s",
  "     --pcfg-pw-complex          |      | Enforce MS password complexity policy                |",
  "     --pcfg-pw-len-max          | Num  | Skip structures with password len > NUM              | --pcfg-pw-len-max=20",
  "     --pcfg-pw-len-min          | Num  | Skip structures with password len < NUM              | --pcfg-pw-len-min=8",
  "     --pcfg-shuffle             |      | Shuffle token order within structures                |",
  "     --pcfg-struct-prob-max     | Num  | Skip structures with probability > NUM%              | --pcfg-struct-prob-max=0.8",
  "     --pcfg-struct-prob-min     | Num  | Skip structures with probability < NUM%              | --pcfg-struct-prob-min=0.1",
  "     --pcfg-terminal-count-min  | Num  | Skip terminals with count < NUM                      | --pcfg-terminal-count-min=5",
  "     --pcfg-token-count-max     | Num  | Max number of tokens per structure                   | --pcfg-token-count-max=5",
  "     --pcfg-token-count-min     | Num  | Min number of tokens per structure                   | --pcfg-token-count-min=3",
  "     --pcfg-token-len-max       | Num  | Max length for each individual token                 | --pcfg-token-len-max=6",
  "     --pcfg-token-len-min       | Num  | Min length for each individual token                 | --pcfg-token-len-min=3",
  "     --pcfg-token-types         | Str  | Filter structures by token types                     | --pcfg-token-types=YC",
  "     --pcfg-train               | File | Path to dictionary file for training                 | --pcfg-train=example.dict",
  "     --pcfg-train-af-disable    |      | Disable training Admission Filter                    |",
  "     --pcfg-train-af-threshold  | Num  | Set train Admission Filter threshold (default: 2)    | --pcfg-train-af-threshold=3",
  "     --pcfg-train-df-disable    |      | Disable training Data cleaning Filters               |",
  "     --pcfg-train-format        | Num  | Set train input format (0:wordlist, 1:count:pw)      | --pcfg-train-format=1",
  "",
  NULL
};
...
static const char *const USAGE_BIG_POST_HASHMODES[] =
{
  ...
  "  8  | Generic",
  "  9  | Association",
  "  10 | Probabilistic Context-Free Grammar",
  "",
  "- [ Built-in Charsets ] -",
  "",
  "  ? | Charset",
  ...
  "   # | Mode",
  " ====+======",
  "   0 | Random Weighed (CPU generator)",
  "   1 | AHF - Adaptive Hybrid-Fuzzing (CPU generator)",
  "   2 | Probability (CPU generator)",
  "   3 | Probability (GPU generator)",
  "   4 | OMEN by Cost (CPU generator)",
  "   5 | OMEN by Cost (GPU generator)",
  "   6 | OMEN by Struct (CPU generator)",
  "   7 | OMEN by Struct (GPU generator)",
  "",
  "- [ PCFG OMEN Types ] -",
  "",
  "  # | Type",
  ...
  "  M | Mixed Case           | iPhONE",
  "  C | Capitalized          | Password",
  "  D | Digits               | 123456",
  "  S | Special              | !@#$%",
  "  P | Punctuation          | .,:;",
  "  W | Whitespace           | _ -",
  "  Y | Year (1900-2050)     | 2026",
  "  E | Email Provider       | @gmail.com",
  "  K | Keyboard Walk        | qwerty",
  "  R | Repetitions          | 111111",
  "  Q | Logical Sequence     | abcdef",
  "  J | Emoji                | 🔥😎👽",
  "  A | Latin Ext/Accents    | àêòèâàöèè",
  "  I | Cyrillic             | солнышко",
  ...
  "",
  "- [ PCFG Trainer Input Format ] -",
  "",
  "  # | Description          | Example content",
  " ===+======================+=================",
  "  0 | Simple Wordlist      | password",
  "  1 | Weighed Wordlist     | 100:password",
  "",
  "- [ License ] -",
  "",
  ...
  "  PCFG (AHF)       | MD5   | hashcat -a 10 -m 0 example0.hash --pcfg-model example.dict.pcfg --pcfg-mode 1 -r rules/best66.rule",
  "",
