    ...
    for (int j = 0; j < out_cnt; j++)
    {
      ...
      while (!feof (fp))
      {
        ...
        for (size_t i = line_len - 1; i && cut_tries; i--, line_len--)
        {
          ...
          if (is_salted == true)
          {
            memset (hash_buf.salt, 0, sizeof (salt_t)); // needed ? (let's play it safe!)
          ...
          else
          {
            ...
            for (u32 digest_pos = 0; digest_pos < salt_buf->digests_cnt; digest_pos++, idx++, digests_buf_ptr += dgst_size)
            {
              if (hashes->digests_shown[idx] == 1) continue;

              cracked = (sort_by_digest_p0p1 (digests_buf_ptr, hash_buf.digest, hashconfig) == 0);
