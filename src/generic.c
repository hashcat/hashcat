...
// Where a feed reaches a candidate once the rules are in play: the index of the first base word and
// the rule applied to it that together produce it.
//
// The search above answers about the base word, which is the whole attack only when no rule runs. A
// rule is not invertible - there is no reading of "sa@ $1 c" that turns a candidate back into the
// word it came from - so this one is answered forwards, by making the candidates the run makes and
// comparing them, which is the run's own work minus the hashing.
//
// The two loops are nested the way the run nests them, and that is what makes the first hit the
// answer: -a 0 walks base words on the outside and the whole rule set on the inside, and --skip
// addresses the outer loop. So the search stops at the first pair rather than counting the rest. A
// miss costs the full words x rules, because proving no rule makes the candidate means trying all of
// them.
//
// The rule is applied to the word the transform produced, not to the line, because that is where the
// rule sits in the run: the host applies --hex-wordlist, $HEX[] and the encoding change to the line,
// and the device applies the rule to what comes out. optimized picks the rule engine the run would
// use, since -O runs a second one that does not always agree with the first.

int generic_ctx_word_index_rules (hashcat_ctx_t *hashcat_ctx, const generic_role_t role, const u8 *cand, const u32 cand_len, const kernel_rule_t *rules, const u32 rules_cnt, const bool optimized, u64 *out_index, u64 *out_rule, u8 *out_word, u32 *out_word_len, u64 *out_words)
{
  generic_ctx_t *generic_ctx = &hashcat_ctx->generic_ctx[role];
  ...
  if (rules_cnt == 0) return -1;

  // Same two reasons as generic_ctx_word_index (): a device engine has no device to build on here,
  // and thread slot 0 is free because --lookup borrows --keyspace and no device thread was created.
  ...
  else
  {
    ...
    // The layout the rule engines want: one candidate as u32 words, with the optimized engine
    // addressing it as the two halves the optimized kernels keep it in.

    u32 rule_buf[PW_MAX / sizeof (u32)];

    u8 *const rule_ptr = (u8 *) rule_buf;

    // A word too long for the rule engine is passed over rather than cut to fit. -O keeps a candidate
    // in two 16 byte halves and cannot see past them, and the pure engine is written for a buffer
    // under 256 bytes. A run rejects such a base word before any rule touches it, so cutting it here
    // would build a candidate the run never builds and report it as reached.

    const u32 len_max = (optimized == true) ? 31 : (PW_MAX - 1);
    ...
    while (found == false)
    {
      ...
      const bool usable = (generic_word_transform (&transform, buf, out_len, sizeof (buf), &len) == true) && (len <= len_max);

      if (usable == true)
      {
        for (u32 rule_pos = 0; rule_pos < rules_cnt; rule_pos++)
        {
          memset (rule_buf, 0, sizeof (rule_buf));

          memcpy (rule_ptr, buf, len);

          int rule_len;

          if (optimized == true)
          {
            rule_len = (int) apply_rules_optimized (rules[rule_pos].cmds, &rule_buf[0], &rule_buf[4], len);
          }
          else
          {
            rule_len = apply_rules (rules[rule_pos].cmds, rule_buf, (int) len);
          }

          if (rule_len < 0) continue;

          if ((u32) rule_len != cand_len) continue;

          if (memcmp (rule_ptr, cand, cand_len) != 0) continue;
          ...
          *out_rule  = rule_pos;
          ...
          // The word the rule was handed, which is what the report names it by. What the transform
          // made of the line, not the line, for the same reason the comparison is: that is the string
          // the rule ran on.

          memcpy (out_word, buf, len);
