...
uint SHA256_S0(uint x);
inline uint SHA256_S0(uint x) {
  return rotl32(x, 25u) ^ rotl32(x, 14u) ^ SHIFT_RIGHT_32(x, 3u);
}

uint SHA256_S1(uint x);
inline uint SHA256_S1(uint x) {
  return rotl32(x, 15u) ^ rotl32(x, 13u) ^ SHIFT_RIGHT_32(x, 10u);
}

uint SHA256_S2(uint x);
inline uint SHA256_S2(uint x) {
  return rotl32(x, 30u) ^ rotl32(x, 19u) ^ rotl32(x, 10u);
}

uint SHA256_S3(uint x);
inline uint SHA256_S3(uint x) {
  return rotl32(x, 26u) ^ rotl32(x, 21u) ^ rotl32(x, 7u);
}

uint SHA256_F0(uint x, uint y, uint z);
inline uint SHA256_F0(uint x, uint y, uint z) {
  return (x& y) | (z& (x ^ y));
}

uint SHA256_F1(uint x, uint y, uint z);
inline uint SHA256_F1(uint x, uint y, uint z) {
  return (z ^ (x& (y ^ z)));
}

uint SHA256_F0o(uint x, uint y, uint z);
inline uint SHA256_F0o(uint x, uint y, uint z) {
  return (SHA256_F0(x, y, z));
}

uint SHA256_F1o(uint x, uint y, uint z);
inline uint  SHA256_F1o(uint x, uint y, uint z) {
  return (SHA256_F1(x, y, z));
}
...
uint SHA256_EXPAND(uint x, uint y, uint z, uint w);
inline uint SHA256_EXPAND(uint x, uint y, uint z, uint w) {
  return SHA256_S1(x) + y + SHA256_S0(z) + w;
}
