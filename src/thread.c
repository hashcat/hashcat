 ...
 * #if defined (_WIN)
 * 
 * BOOL WINAPI sigHandler_default (DWORD sig) { switch (sig) { case CTRL_CLOSE_EVENT:
 * 
 * * * special case see: https://stackoverflow.com/questions/3640633/c-setconsolectrlhandler-routine-issue/5610042#5610042 * if the user interacts w/ the user-interface (GUI/cmd), we need to do the finalization job within this signal handler * function otherwise it is too late (e.g. after returning from this function) *
 * 
 ...
 * return TRUE;
 * 
 * case CTRL_C_EVENT: case CTRL_LOGOFF_EVENT: case CTRL_SHUTDOWN_EVENT:
 * 
 * myabort (hashcat_ctx->status_ctx);
 * 
 * SetConsoleCtrlHandler (NULL, TRUE);
 ...
 * return FALSE; }
 * 
 * BOOL WINAPI sigHandler_benchmark (DWORD sig) { switch (sig) { case CTRL_CLOSE_EVENT:
 * 
 ...
 * return FALSE; }
 * 
 * void hc_signal (BOOL WINAPI (callback) (DWORD)) { if (callback == NULL) { SetConsoleCtrlHandler ((PHANDLER_ROUTINE) callback, FALSE); } else { SetConsoleCtrlHandler ((PHANDLER_ROUTINE) callback, TRUE); } }
 * 
 * #else
 * 
 * void sigHandler_default (int sig) { myabort (hashcat_ctx->status_ctx);
 * 
 * signal (sig, NULL); }
 * 
 * void sigHandler_benchmark (int sig) { myquit (hashcat_ctx->status_ctx);
 * 
 * signal (sig, NULL); }
 * 
 * void hc_signal (void (callback) (int)) { if (callback == NULL) callback = SIG_DFL;
 * 
 * signal (SIGINT, callback); signal (SIGTERM, callback); signal (SIGABRT, callback); }
 * 
 * #endif */
