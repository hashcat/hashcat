...
static void sha256_transform (const u32x w0[4], const u32x w1[4], const u32x w2[4], const u32x w3[4], u32x digest[8])
{
  ...
  u32x we_t = swap_workaround (w3[2]);
  u32x wf_t = swap_workaround (w3[3]);
...
static void sha384_transform (const u64 w0[4], const u64 w1[4], const u64 w2[4], const u64 w3[4], u64x digest[8])
{
  u64x a = digest[0];
  u64x b = digest[1];
  u64x c = digest[2];
  ...
  u64x w0_t = swap_workaround (w0[0]);
  u64x w1_t = swap_workaround (w0[1]);
  u64x w2_t = swap_workaround (w0[2]);
  u64x w3_t = swap_workaround (w0[3]);
  u64x w4_t = swap_workaround (w1[0]);
  u64x w5_t = swap_workaround (w1[1]);
  u64x w6_t = swap_workaround (w1[2]);
  u64x w7_t = swap_workaround (w1[3]);
  u64x w8_t = swap_workaround (w2[0]);
  u64x w9_t = swap_workaround (w2[1]);
  u64x wa_t = swap_workaround (w2[2]);
  u64x wb_t = swap_workaround (w2[3]);
  u64x wc_t = swap_workaround (w3[0]);
  u64x wd_t = swap_workaround (w3[1]);
  u64x we_t = swap_workaround (w3[2]);
  u64x wf_t = swap_workaround (w3[3]);
...
__kernel void __attribute__((reqd_work_group_size (64, 1, 1))) m10700_comp (__global pw_t *pws, __global gpu_rule_t *rules_buf, __global comb_t *combs_buf, __global bf_t *bfs_buf, __global pdf17l8_tmp_t *tmps, __global void *hooks, __global u32 *bitmaps_buf_s1_a, __global u32 *bitmaps_buf_s1_b, __global u32 *bitmaps_buf_s1_c, __global u32 *bitmaps_buf_s1_d, __global u32 *bitmaps_buf_s2_a, __global u32 *bitmaps_buf_s2_b, __global u32 *bitmaps_buf_s2_c, __global u32 *bitmaps_buf_s2_d, __global plain_t *plains_buf, __global digest_t *digests_buf, __global u32 *hashes_shown, __global salt_t *salt_bufs, __global pdf_t *pdf_bufs, __global u32 *d_return_buf, __global u32 *d_scryptV_buf, const u32 bitmap_mask, const u32 bitmap_shift1, const u32 bitmap_shift2, const u32 salt_pos, const u32 loop_pos, const u32 loop_cnt, const u32 rules_cnt, const u32 digests_cnt, const u32 digests_offset, const u32 combs_mode, const u32 gid_max)
{
  ...
  const u32x r0 = swap_workaround (tmps[gid].dgst32[DGST_R0]);
  const u32x r1 = swap_workaround (tmps[gid].dgst32[DGST_R1]);
  const u32x r2 = swap_workaround (tmps[gid].dgst32[DGST_R2]);
  const u32x r3 = swap_workaround (tmps[gid].dgst32[DGST_R3]);
