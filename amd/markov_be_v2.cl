...
__kernel void __attribute__((reqd_work_group_size (64, 1, 1))) r_markov (__global bf_t *pws_buf_r, __global cs_t *root_css_buf, __global cs_t *markov_css_buf, const u64 off, const u32 pw_r_len, const u32 mask80, const u32 bits14, const u32 bits15, const u32 gid_max)
{
  ...
  generate_pw (pw_buf, root_css_buf, markov_css_buf, pw_r_len, 0, 0, 0, 0, off + gid2 + 2);

  pws_buf_r[gid2 + 2].i = pw_buf[0];

  generate_pw (pw_buf, root_css_buf, markov_css_buf, pw_r_len, 0, 0, 0, 0, off + gid2 + 3);

  pws_buf_r[gid2 + 3].i = pw_buf[0];
}
...
__kernel void __attribute__((reqd_work_group_size (64, 1, 1))) C_markov (__global comb_t *pws_buf, __global cs_t *root_css_buf, __global cs_t *markov_css_buf, const u64 off, const u32 pw_len, const u32 mask80, const u32 bits14, const u32 bits15, const u32 gid_max)
{
  ...
  generate_pw (pw_buf, root_css_buf, markov_css_buf, pw_len, 0, mask80, bits14, bits15, off + gid2 + 2);

  pws_buf[gid2 + 2].i[ 0] = pw_buf[ 0];
  pws_buf[gid2 + 2].i[ 1] = pw_buf[ 1];
  pws_buf[gid2 + 2].i[ 2] = pw_buf[ 2];
  pws_buf[gid2 + 2].i[ 3] = pw_buf[ 3];
  pws_buf[gid2 + 2].i[ 4] = pw_buf[ 4];
  pws_buf[gid2 + 2].i[ 5] = pw_buf[ 5];
  pws_buf[gid2 + 2].i[ 6] = pw_buf[ 6];
  pws_buf[gid2 + 2].i[ 7] = pw_buf[ 7];

  pws_buf[gid2 + 2].pw_len = pw_len;

  generate_pw (pw_buf, root_css_buf, markov_css_buf, pw_len, 0, mask80, bits14, bits15, off + gid2 + 3);

  pws_buf[gid2 + 3].i[ 0] = pw_buf[ 0];
  pws_buf[gid2 + 3].i[ 1] = pw_buf[ 1];
  pws_buf[gid2 + 3].i[ 2] = pw_buf[ 2];
  pws_buf[gid2 + 3].i[ 3] = pw_buf[ 3];
  pws_buf[gid2 + 3].i[ 4] = pw_buf[ 4];
  pws_buf[gid2 + 3].i[ 5] = pw_buf[ 5];
  pws_buf[gid2 + 3].i[ 6] = pw_buf[ 6];
  pws_buf[gid2 + 3].i[ 7] = pw_buf[ 7];

  pws_buf[gid2 + 3].pw_len = pw_len;
}
