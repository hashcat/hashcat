...
# Bitcoin WIF, uncompressed P2PKH. The password is a WIF private key; the hash is the legacy address
# derived from it. The random password is a BIP32 key at m/0' off a seed, exported as uncompressed WIF.
...
def module_get_random_password(word):
  # BIP32 master from the seed, then the m/0' child, exported as an uncompressed WIF

  key, chain_code = _bip32_from_seed(word)
  key, chain_code = _bip32_derive_hardened(key, chain_code, 0)

  return _wif_from_private(key.to_bytes(32, "big"), False).encode("ascii")
