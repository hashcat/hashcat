...
# DPAPI masterkey v2. Same construction as v1 (mode 15300) but the default salt version is 2, so the
# masterkey blob is SHA512 based and AES-256-CBC encrypted. The password becomes an NT hash (SHA1 for
# context 1, MD4 for context 2), HMAC-SHA1 with the SID derives a key, and a Microsoft variant of
# PBKDF2 stretches it. The optimized kernels widen the password byte by byte instead of decoding
# UTF-8, so the oracle picks its charset from IS_OPTIMIZED the way test.sh sets it.
...
def module_generate_hash(word, salt_buf, dpapimk_salt=None, cipher=None):
  ...
  pw16 = word.decode(PW_CHARSET).encode("utf-16-le")

  if context == 1:
    user_hash = hashlib.sha1(pw16).digest()
  elif context == 2:
    user_hash = MD4.new(pw16).digest()
