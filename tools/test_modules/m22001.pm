...
# WPA-PMK-PMKID+EAPOL (mode 22001): identical to 22000 but the candidate is the
# 32-byte PMK itself (64 hex chars), not a passphrase, so there is no PBKDF2
# and the ESSID is not part of the key derivation (it is still carried in the
# record). Both record kinds are emitted: type 1 = PMKID, type 2 = EAPOL MIC
# (keyver 1/2/3 -> MD5/SHA1/AES-CMAC). pw is fixed at 64 (pw_min==pw_max).
...
sub module_generate_hash
{
  ...
  # the candidate is the PMK as 64 hex chars -> 32 raw bytes
  my $pmk = pack ("H*", $word);
