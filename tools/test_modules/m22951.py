...
# RSA/DSA/EC/OpenSSH private key, PEM. cid 5 is AES-256 CBC, key is a two-round OpenSSL EVP_BytesToKey MD5 (32 bytes), iv is the salt.
...
def module_constraints():
  return [[0, 128], [32, 32], [-1, -1], [-1, -1], [-1, -1]]
...
def module_generate_hash(word, salt, cid=None, data=None):
  ...
  if data is not None:
    ...
    dec_bin = _std_unpad(AES.new(key, AES.MODE_CBC, salt_bin).decrypt(data_bin))
  ...
  enc_bin = AES.new(key, AES.MODE_CBC, salt_bin).encrypt(pad(data_bin, BS))
