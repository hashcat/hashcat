...
from lib.test_helpers import random_number, random_bytes, pack_hex

# WPA-PMK-PMKID+EAPOL. Identical to mode 22000 except the candidate is the 32 byte
# PMK itself (64 hex chars), so there is no PBKDF2 and the ESSID does not enter the
# key derivation (it is still carried in the record). Type 1 is a PMKID, type 2 is
# an EAPOL MIC (keyver 1/2/3 -> HMAC-MD5, HMAC-SHA1, AES-CMAC).
...
def module_generate_hash(word, salt=None, type=None, macap=None, macsta=None,
  ...
  # the candidate is the PMK as 64 hex chars -> 32 raw bytes, via perl's lenient pack("H*")
  pmk = pack_hex(word)
