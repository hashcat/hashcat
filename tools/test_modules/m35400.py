...
# Kerberos 5 AS-REP (krb5asrep, etype 23), keyed straight from the NT hash. Same RC4-HMAC schedule as
# m18200, except the password field is the NT hash as hex32, so the key is that hex unpacked rather
# than MD4 of the UTF-16 password. On verify the decrypted ticket is kept only when its ASN.1 markers
...
def module_generate_hash(word, salt=None, user_principal_name=None, checksum=None, edata2=None):
...
def module_verify_hash(line):
  ...
  if len(data) != 4 or data[1] != "krb5asrep" or data[2] != "23":
    return None
