...
# RSA/DSA/EC/OpenSSH private key, PEM. cid 6 is DES CBC, key derived by the OpenSSL EVP_BytesToKey
# MD5 scheme (single round, 8 bytes), iv is the salt.
...
def generate_key(word, salt_bin):
  ...
  return hashlib.md5(word + salt8).digest()[:8]
...
def module_generate_hash(word, salt, cid=None, data=None):
  ...
  if data is not None:
    ...
    dec_bin = _std_unpad(DES.new(key, DES.MODE_CBC, salt_bin).decrypt(data_bin))
  ...
  enc_bin = DES.new(key, DES.MODE_CBC, salt_bin).encrypt(pad(data_bin, BS))
