...
'32313131363134323835355aa10502030c28a2';
  ...
  if ($format == 3)
  {
    $tmp_hash = sprintf ('$krb5pa$18$%s$%s$%s$%s%s', $user, $realm, $salt, unpack ("H*", $enc_timestamp), $checksum);
  }
  elsif ($format == 2)
  {
    $tmp_hash = sprintf ('$krb5pa$18$%s$%s$$%s%s', $user, $realm, unpack ("H*", $enc_timestamp), $checksum);
  }
  else
  {
    $tmp_hash = sprintf ('$krb5pa$18$%s$%s$%s%s', $user, $realm, unpack ("H*", $enc_timestamp), $checksum);
