...
sub module_generate_hash
{
  ...
  ## the collider digest is the 40 bit RC4 key itself, the first 5 bytes of the encryption key

  my $rc4key = substr ($res, 0, 5);

  my $m = Crypt::RC4->new ($rc4key);

  $u = $m->RC4 ($padding);

  my $hash = sprintf ('$pdf$%d*%d*40*%d*%d*16*%s*32*%s*32*%s:%s', 1, 2, $P, 0, $id, unpack ("H*", $u), $o, unpack ("H*", $rc4key));
...
sub module_verify_hash
{
  ...
  ## the hash carries a ':' of its own, the rc4key digest sits between it and the password
