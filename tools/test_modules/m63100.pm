...
sub module_generate_hash
{
  ...
  {
    $hash_buf = sha256 ($word . $hash_buf);
  }

  $hash_buf = encode_base64 ($hash_buf . $salt, "");

  my $hash = sprintf ("{x-isSHA256, %i}%s", $iter, $hash_buf);
...
sub module_verify_hash
{
  ...
  return unless (substr ($hash_in, 0, 13) eq '{x-isSHA256, ');
  ...
  my $salt = substr ($base64_decoded, 32);
