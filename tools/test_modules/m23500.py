...
# AxCrypt 2 AES-128. PBKDF2-HMAC-SHA512 gives a 64 byte key that is folded to 16 bytes and xored
...
def module_generate_hash(word, salt_wrap, iter_wrap=None, data=None, salt_kdf=None, iter_kdf=None):
  ...
  # fold the 64 byte key down to the 16 byte AES key
  kek = _xor(_xor(kdf[0:16], kdf[16:32]), _xor(kdf[32:48], kdf[48:64]))
  kek = _xor(kek, salt_wrap[0:16])
