...
# Kerberos 5 etype 18 (AES256-CTS key from the DB): PBKDF2-HMAC-SHA1 of the password
...
def module_generate_hash(word, salt, iterations=None, user="user", realm="realm"):
  ...
  seed = hashlib.pbkdf2_hmac("sha1", word, mysalt, 4096, 32)

  iv = b"\x00" * 16

  key = AES.new(seed, AES.MODE_CBC, iv).encrypt(NFOLD)

  key += AES.new(seed, AES.MODE_CBC, iv).encrypt(key)

  return "$krb5db$18$%s$%s$%s" % (user, realm, key.hex())
...
def module_verify_hash(line):
  ...
  if len(fields) < 5 or fields[1] != "krb5db" or fields[2] != "18":
    return None
