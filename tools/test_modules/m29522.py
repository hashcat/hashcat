...
# LUKS container mode. The oracle builds a real container by running luks1.sh with this mode's id and
...
_SCRIPT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "luks1.sh")
...
def module_generate_hash(word, salt=None, iterations=None):
  out = subprocess.run([_SCRIPT, _MODULE_ID, word], capture_output=True).stdout
