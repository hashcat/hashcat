...
sub module_generate_hash
{
  ...
  my $digest = hmac ($nr, $word, \&sha256, 64);
  $digest    = hmac_hex ($msg, $digest, \&sha256, 64);
...
sub module_verify_hash
{
  ...
  my $sep_pos = rindex ($line, ':');

  return if $sep_pos < 0;

  my $hash = substr ($line, 0,      $sep_pos);
  my $word = substr ($line, $sep_pos + 1);

  my @data = split (':', $hash);

  return unless scalar @data == 9;

  my $salt = join (':', @data[0 .. 7]);
