...
# Kerberos 5 TGS-REP (krb5tgs, etype 23). A fixed cleartext ticket is RC4 encrypted under the key the
# NT hash derives; on verify the stored edata2 is decrypted and kept only when its ASN.1 markers
# check out, so a wrong password does not round trip into a false positive.
...
def module_generate_hash(word, salt=None, iterations=None, user=None, realm=None, spn=None,
