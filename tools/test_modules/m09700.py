...
# MS Office <= 2003 MD5 + RC4: the key comes from md5 of the UTF-16LE password, mixed 16 times with
# the salt, and the version bit is read from it. See lib/oldoffice.py.
...
def module_generate_hash(word, salt, iterations=None, param=None, version=None):
  ...
  tmp = hashlib.md5(tmp * 16 if False else (tmp + salt_bin) * 16).digest()[:5]
