...
# DPAPI masterkey v1 with a domain backup key (context 3). The password becomes an MD4 NT hash, two
# PBKDF2-HMAC-SHA256 rounds derive the user hash, HMAC-SHA1 with the SID derives a key, and a
# Microsoft variant of PBKDF2 stretches it before the 3DES-EDE-CBC masterkey blob. The optimized
# kernels widen the password byte by byte instead of decoding UTF-8, so the oracle picks its charset
# from IS_OPTIMIZED the way test.sh sets it.
...
def module_generate_hash(word, salt_buf, dpapimk_salt=None, cipher=None):
  if dpapimk_salt is None:
    dpapimk_salt = get_random_dpapimk_salt(1)
  ...
  # context 3 derives the user hash through the domain backup key path: MD4 NT hash, then two
  # PBKDF2-HMAC-SHA256 rounds keyed by the SID.
