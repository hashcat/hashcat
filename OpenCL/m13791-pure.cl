...
KERNEL_FQ KERNEL_FA void m13791_init (KERN_ATTR_TMPS_ESALT (vc_tmp_t, vc_t))
{
...
KERNEL_FQ KERNEL_FA void m13791_loop (KERN_ATTR_TMPS_ESALT (vc_tmp_t, vc_t))
{
  const u64 gid = get_global_id (0);
...
KERNEL_FQ KERNEL_FA void m13791_loop_extended (KERN_ATTR_TMPS_ESALT (vc_tmp_t, vc_t))
{
  const u64 gid = get_global_id (0);
  const u64 lid = get_local_id (0);
  const u64 lsz = get_local_size (0);
...
KERNEL_FQ KERNEL_FA void m13791_comp (KERN_ATTR_TMPS_ESALT (vc_tmp_t, vc_t))
{
  const u64 gid = get_global_id (0);
  const u64 lid = get_local_id (0);
  const u64 lsz = get_local_size (0);
