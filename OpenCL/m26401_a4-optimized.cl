...
DECLSPEC bool pcfg_aes128_ecb (PRIVATE_AS const u32 *ukey, PRIVATE_AS u32 *dgst, PRIVATE_AS const pcfg_hash_ctx_t *hc)
{
  #ifdef REAL_SHM
  ...
  aes128_encrypt (ks, hc->pt, ct, s_te0, s_te1, s_te2, s_te3, s_te4);
...
DECLSPEC bool pcfg_hash (PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_aes128_ecb (w, dgst, hc);
}
...
DECLSPEC bool pcfg_hash_global (PRIVATE_AS const pcfg_hash_ctx_t *hc, GLOBAL_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  for (u32 i = 0; i < 4; i++) ukey[i] = ((i * 4) < len) ? w[i] : 0;

  return pcfg_aes128_ecb (ukey, dgst, hc);
}

#define PCFG_KERNEL_MXX m26401_mxx
#define PCFG_KERNEL_SXX m26401_sxx
