...
// AES-192 expands into 13 round keys of four words, the same KEYLEN the rules kernels use.

#define KEYLEN 52

// Only the encryption tables are needed here, the mode never decrypts.
...
DECLSPEC bool pcfg_aes192_ecb (PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS const u32 *ukey, PRIVATE_AS u32 *dgst)
{
  #ifdef REAL_SHM
  ...
  aes192_encrypt (ks, hc->pt, ct, s_te0, s_te1, s_te2, s_te3, s_te4);
...
DECLSPEC bool pcfg_hash (PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_aes192_ecb (hc, ukey, dgst);
}
...
DECLSPEC bool pcfg_hash_global (PRIVATE_AS const pcfg_hash_ctx_t *hc, GLOBAL_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  for (u32 i = 0; i < 6; i++) ukey[i] = ((i * 4) < len) ? w[i] : 0;

  return pcfg_aes192_ecb (hc, ukey, dgst);
}

#define PCFG_KERNEL_MXX m26402_mxx
#define PCFG_KERNEL_SXX m26402_sxx
