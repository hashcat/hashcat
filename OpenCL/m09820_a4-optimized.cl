...
DECLSPEC bool pcfg_oldoffice34 (PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  dgst[0] = hc_swap32_S (digest[0]);
  dgst[1] = hc_swap32_S (digest[1]) & 0xff;
  dgst[2] = 0;
  dgst[3] = 0;

  if (hc->secondBlockLen == 0) return true;

  w1[1] = 0x01000000;

  u32 block_hash[5];

  block_hash[0] = SHA1M_A;
  block_hash[1] = SHA1M_B;
  block_hash[2] = SHA1M_C;
  block_hash[3] = SHA1M_D;
  block_hash[4] = SHA1M_E;

  sha1_transform (w0, w1, w2, w3, block_hash);

  u32 key[4];

  key[0] = hc_swap32_S (block_hash[0]);
  key[1] = hc_swap32_S (block_hash[1]) & 0xff; // only 40-bit key
  key[2] = 0;
  key[3] = 0;
  ...
  // the keystream carries on from where the first sixteen bytes left it off, so i and j both continue
  ...
  if (null_bytes < MIN_NULL_BYTES) return false;
...
#define PCFG_KERNEL_MXX m09820_mxx
#define PCFG_KERNEL_SXX m09820_sxx
