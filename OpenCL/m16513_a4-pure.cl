...
DECLSPEC bool pcfg_hash (PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  sha512_hmac_init_swap (&ctx, w, len);

  sha512_hmac_update_global_swap (&ctx, hc->esalt_bufs[hc->digest_pos].salt_buf, hc->esalt_bufs[hc->digest_pos].salt_len);
...
DECLSPEC bool pcfg_hash_global (PRIVATE_AS const pcfg_hash_ctx_t *hc, GLOBAL_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  sha512_hmac_init_global_swap (&ctx, w, len);
...
#define PCFG_KERNEL_MXX m16513_mxx
#define PCFG_KERNEL_SXX m16513_sxx
