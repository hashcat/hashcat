...
DECLSPEC void hmac_blake2s_run_V (PRIVATE_AS u32x *m, PRIVATE_AS u32x *ipad, PRIVATE_AS u32x *opad, PRIVATE_AS u32x *digest)
{
  digest[0] = ipad[0];
  digest[1] = ipad[1];
  digest[2] = ipad[2];
  ...
  // 96 = 64 + 32
  blake2s_transform_vector (digest, m, 96, BLAKE2S_FINAL);
...
KERNEL_FQ KERNEL_FA void m13792_init (KERN_ATTR_TMPS_ESALT (vc_tmp_t, vc_t))
{
  ...
  blake2s_hmac_ctx_t blake2s_hmac_ctx;
  
  blake2s_hmac_init (&blake2s_hmac_ctx, w, pw_len);
  ...
  tmps[gid].ipad[2] = blake2s_hmac_ctx.ipad.h[2];
  tmps[gid].ipad[3] = blake2s_hmac_ctx.ipad.h[3];
  tmps[gid].ipad[4] = blake2s_hmac_ctx.ipad.h[4];
  tmps[gid].ipad[5] = blake2s_hmac_ctx.ipad.h[5];
  tmps[gid].ipad[6] = blake2s_hmac_ctx.ipad.h[6];
  tmps[gid].ipad[7] = blake2s_hmac_ctx.ipad.h[7];

  tmps[gid].opad[0] = blake2s_hmac_ctx.opad.h[0];
  tmps[gid].opad[1] = blake2s_hmac_ctx.opad.h[1];
  tmps[gid].opad[2] = blake2s_hmac_ctx.opad.h[2];
  tmps[gid].opad[3] = blake2s_hmac_ctx.opad.h[3];
  tmps[gid].opad[4] = blake2s_hmac_ctx.opad.h[4];
  tmps[gid].opad[5] = blake2s_hmac_ctx.opad.h[5];
  tmps[gid].opad[6] = blake2s_hmac_ctx.opad.h[6];
  tmps[gid].opad[7] = blake2s_hmac_ctx.opad.h[7];

  blake2s_hmac_update_global (&blake2s_hmac_ctx2, salt_bufs[SALT_POS_HOST].salt_buf, VC_SALT_LEN); // no need for swap

  for (u32 i = 0, j = 1; i < 32; i += 8, j += 1)
  {
    blake2s_hmac_ctx = blake2s_hmac_ctx2;
    ...
    blake2s_hmac_update_64 (&blake2s_hmac_ctx, w0, w1, w2, w3, 4);

    blake2s_hmac_final (&blake2s_hmac_ctx);

    tmps[gid].dgst[i + 0] = blake2s_hmac_ctx.opad.h[0];
    tmps[gid].dgst[i + 1] = blake2s_hmac_ctx.opad.h[1];
    tmps[gid].dgst[i + 2] = blake2s_hmac_ctx.opad.h[2];
    tmps[gid].dgst[i + 3] = blake2s_hmac_ctx.opad.h[3];
    tmps[gid].dgst[i + 4] = blake2s_hmac_ctx.opad.h[4];
    tmps[gid].dgst[i + 5] = blake2s_hmac_ctx.opad.h[5];
    tmps[gid].dgst[i + 6] = blake2s_hmac_ctx.opad.h[6];
    tmps[gid].dgst[i + 7] = blake2s_hmac_ctx.opad.h[7];
...
KERNEL_FQ KERNEL_FA void m13792_loop (KERN_ATTR_TMPS_ESALT (vc_tmp_t, vc_t))
{
  const u64 gid = get_global_id (0);
  ...
  for (u32 i = 0; i < 32; i += 8)
  {
    ...
    for (u32 j = 0; j < LOOP_CNT; j++)
    {
      ...
      hmac_blake2s_run_V (m, ipad, opad, dgst);
...
KERNEL_FQ KERNEL_FA void m13792_loop_extended (KERN_ATTR_TMPS_ESALT (vc_tmp_t, vc_t))
{
  const u64 gid = get_global_id (0);
  const u64 lid = get_local_id (0);
  const u64 lsz = get_local_size (0);
...
KERNEL_FQ KERNEL_FA void m13792_comp (KERN_ATTR_TMPS_ESALT (vc_tmp_t, vc_t))
{
  const u64 gid = get_global_id (0);
  const u64 lid = get_local_id (0);
  const u64 lsz = get_local_size (0);
