  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos++)
  {
    ...
    if (COMBS_IS_MIDDLE)
    {
      if (COMBS_PRE (il_pos).pw_len > 0)
      {
        ...
        ripemd160_update_global (&ctx, COMBS_PRE (il_pos).i, COMBS_PRE (il_pos).pw_len);
        ripemd160_update_global (&ctx, pws[gid].i, pws[gid].pw_len);
      }

      if (COMBS_MID  (il_pos).pw_len > 0) ripemd160_update_global (&ctx, COMBS_MID  (il_pos).i, COMBS_MID  (il_pos).pw_len);
      if (COMBS_WORD (il_pos).pw_len > 0) ripemd160_update_global (&ctx, COMBS_WORD (il_pos).i, COMBS_WORD (il_pos).pw_len);
    }

    ripemd160_update_global (&ctx, COMBS_POST (il_pos).i, COMBS_POST (il_pos).pw_len);
