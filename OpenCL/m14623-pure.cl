...
KERNEL_FQ void HC_ATTR_SEQ m14623_init (KERN_ATTR_TMPS_ESALT (luks_tmp_t, luks_t))
{
  /**
   * base
   */
...
KERNEL_FQ void HC_ATTR_SEQ m14623_loop (KERN_ATTR_TMPS_ESALT (luks_tmp_t, luks_t))
{
  const u64 gid = get_global_id (0);
...
KERNEL_FQ void HC_ATTR_SEQ m14623_comp (KERN_ATTR_TMPS_ESALT (luks_tmp_t, luks_t))
{
  const u64 gid = get_global_id (0);
  ...
  luks_af_sha256_then_twofish_decrypt (&esalt_bufs[DIGESTS_OFFSET_HOST], &tmps[gid], pt_buf);
