...
DECLSPEC void pcfg_hash_init (PRIVATE_AS pcfg_hash_ctx_t *hc, GLOBAL_AS const salt_t *salt_bufs, const u32 salt_pos, MAYBE_UNUSED GLOBAL_AS const void *esalt_bufs, MAYBE_UNUSED GLOBAL_AS const digest_t *digests_buf, MAYBE_UNUSED const u32 digest_pos)
{
  ...
  sha1_update_global (&hc->ctx00, salt_bufs[salt_pos].salt_buf_pc, salt_bufs[salt_pos].salt_len_pc);

  md5_init (&hc->ctx11);

  md5_update_global (&hc->ctx11, salt_bufs[salt_pos].salt_buf, salt_bufs[salt_pos].salt_len);
}
...
#define PCFG_KERNEL_MXX m21300_mxx
#define PCFG_KERNEL_SXX m21300_sxx
