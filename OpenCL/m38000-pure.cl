...
KERNEL_FQ KERNEL_FA void m38000_init (KERN_ATTR_TMPS (mikrotik_ecsrp5_tmp_t))
{
   ...
   *
   * Feed "username:" and the password as two separate updates so no single
   * buffer has to hold the whole string. The context keeps the partial block
  ...
  // "username:" (LE u32 like salt_buf_pc; 255 username bytes + colon = 256 max)
  u32 uc_buf[64];

  for (u32 i = 0; i < 64; i++) uc_buf[i] = 0;

  for (u32 i = 0; i < (username_len + 3) / 4; i++)
  {
    uc_buf[i] = salt_bufs[SALT_POS_HOST].salt_buf_pc[i];
  }
  ...
  // Append ":" at byte position username_len
  {
    ...
    uc_buf[wi] |= ((u32) 0x3a) << (bi * 8);
  }

  sha256_update_swap (&ctx, uc_buf, username_len + 1);
  ...
  sha256_update_swap (&ctx, pw_buf, pw_len);
...
KERNEL_FQ KERNEL_FA void m38000_loop (KERN_ATTR_TMPS (mikrotik_ecsrp5_tmp_t))
{
  // No-op: single-pass algorithm, ECC computed in comp
}

KERNEL_FQ KERNEL_FA void m38000_comp (KERN_ATTR_TMPS (mikrotik_ecsrp5_tmp_t))
{
  const u64 gid = get_global_id (0);
