 ...
 * License.....: MIT
 *
 * MultiBit Classic .wallet - bridge-compatible kernel for scrypt-jane bridge
 */
...
KERNEL_FQ KERNEL_FA void m70600_init (KERN_ATTR_TMPS (scrypt_tmp_t))
{
  ...
  // Salt: 8 bytes from salt_buf, swap32'd
  u32 s[16] = { 0 };
  ...
  // init_swap because key is private and not yet swapped for HMAC
  // update_swap because salt is private and not yet swapped
...
KERNEL_FQ KERNEL_FA void m70600_loop (KERN_ATTR_TMPS (scrypt_tmp_t))
{
  // EMPTY - The scrypt-jane bridge replaces this
}

KERNEL_FQ KERNEL_FA void m70600_comp (KERN_ATTR_TMPS (scrypt_tmp_t))
{
  const u64 gid = get_global_id (0);
  ...
  // HMAC init with private UTF16BE password (swap because private buffer)
