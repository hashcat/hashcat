...
KERNEL_FQ KERNEL_FA void m63300_mxx (KERN_ATTR_RULES_ESALT (racf_ph_t))
{
  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos++)
  {
    ...
    if (tmp.pw_len != ph_pw_len) continue;

    const u32 pw_u32_cnt = (tmp.pw_len + 3) / 4;

    u32 pw_ebc[26];

    for (u32 i = 0; i < num_blocks * 2; i++)
    {
      pw_ebc[i] = 0x40404040;
    }
    ...
    {
      pw_ebc[i] = ascii_to_ebcdic_u32 (tmp.i[i], c_ascii_to_ebcdic);
    }

    const u32 rem = tmp.pw_len & 3;
    ...
    {
      u32 e0 = pw_ebc[block * 2 + 0];
      u32 e1 = pw_ebc[block * 2 + 1];

      u32 k0 = ((e0 ^ prev[0] ^ 0x55555555) & 0x7f7f7f7f) << 1;
      u32 k1 = ((e1 ^ prev[1] ^ 0x55555555) & 0x7f7f7f7f) << 1;
