...
DECLSPEC void pcfg_hash_init (PRIVATE_AS pcfg_hash_ctx_t *hc, GLOBAL_AS const salt_t *salt_bufs, const u32 salt_pos, MAYBE_UNUSED GLOBAL_AS const void *esalt_bufs, MAYBE_UNUSED GLOBAL_AS const digest_t *digests_buf, MAYBE_UNUSED const u32 digest_pos)
{
  ...
  sha384_update_global_swap (&hc->ctx0, salt_bufs[salt_pos].salt_buf, salt_bufs[salt_pos].salt_len);
}
...
{
  sha384_ctx_t ctx = hc->ctx0;

  sha384_update_swap (&ctx, w, len);
...
DECLSPEC bool pcfg_hash_global (PRIVATE_AS const pcfg_hash_ctx_t *hc, GLOBAL_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  sha384_update_global_swap (&ctx, w, len);
...
#define PCFG_KERNEL_MXX m10820_mxx
#define PCFG_KERNEL_SXX m10820_sxx
