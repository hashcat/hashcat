...
DECLSPEC u64 crc64jones_g (GLOBAL_AS const u32 *w, const u32 pw_len, const u64 iv, SHM_TYPE u64 *s_crc64jonestab)
{
  u64 a = iv;
  ...
  for (int i = 0; i < pw_len; i++)
  {
    ...
    a = round_crc64jones (a, v, s_crc64jonestab);
  }

  return a;
}

KERNEL_FQ void HC_ATTR_SEQ m28000_mxx (KERN_ATTR_ESALT (crc64_t))
{
  /**
   * modifier
   */
  ...
  u64 a_ref = crc64jones_g (pws[gid].i, pws[gid].pw_len, iv, s_crc64jonestab);
  ...
  {
    u64 a = crc64jones_g (combs_buf[il_pos].i, combs_buf[il_pos].pw_len, a_ref, s_crc64jonestab);
...
KERNEL_FQ void HC_ATTR_SEQ m28000_sxx (KERN_ATTR_ESALT (crc64_t))
{
  /**
   * modifier
   */
