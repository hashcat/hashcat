...
DECLSPEC void pcfg_hash_init (PRIVATE_AS pcfg_hash_ctx_t *hc, GLOBAL_AS const salt_t *salt_bufs, const u32 salt_pos, MAYBE_UNUSED GLOBAL_AS const void *esalt_bufs, MAYBE_UNUSED GLOBAL_AS const digest_t *digests_buf, MAYBE_UNUSED const u32 digest_pos)
{
  ...
  {
    hc->s[idx] = hc_swap32 (salt_bufs[salt_pos].salt_buf[idx]);
  }

  whirlpool_init (&hc->ctx0, s_MT0, s_MT1, s_MT2, s_MT3, s_MT4, s_MT5, s_MT6, s_MT7);

  whirlpool_update (&hc->ctx0, hc->s, hc->salt_len);
}
...
DECLSPEC bool pcfg_hash (PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  whirlpool_ctx_t ctx = hc->ctx0;

  whirlpool_update_swap (&ctx, w, len);

  whirlpool_update (&ctx, hc->s, hc->salt_len);
...
#define PCFG_KERNEL_MXX m32600_mxx
#define PCFG_KERNEL_SXX m32600_sxx
