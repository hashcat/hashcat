...
KERNEL_FQ void HC_ATTR_SEQ m14642_init (KERN_ATTR_TMPS_ESALT (luks_tmp_t, luks_t))
{
  /**
   * base
   */
...
KERNEL_FQ void HC_ATTR_SEQ m14642_loop (KERN_ATTR_TMPS_ESALT (luks_tmp_t, luks_t))
{
  const u64 gid = get_global_id (0);
...
KERNEL_FQ void HC_ATTR_SEQ m14642_comp (KERN_ATTR_TMPS_ESALT (luks_tmp_t, luks_t))
{
  const u64 gid = get_global_id (0);
  ...
  luks_af_ripemd160_then_serpent_decrypt (&esalt_bufs[DIGESTS_OFFSET_HOST], &tmps[gid], pt_buf);
