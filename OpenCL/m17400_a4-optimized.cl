...
DECLSPEC bool pcfg_sha3_256 (PRIVATE_AS const u32 *w_in, const u32 len, PRIVATE_AS u32 *dgst)
{
  u32 w[8];

  for (u32 i = 0; i < 8; i++) w[i] = w_in[i];

  w[len / 4] |= 0x06u << ((len & 3) * 8);
...
DECLSPEC bool pcfg_hash (MAYBE_UNUSED PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_sha3_256 (w, len, dgst);
}
...
DECLSPEC bool pcfg_hash_global (MAYBE_UNUSED PRIVATE_AS const pcfg_hash_ctx_t *hc, GLOBAL_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_sha3_256 (t, len, dgst);
}

#define PCFG_KERNEL_MXX m17400_mxx
#define PCFG_KERNEL_SXX m17400_sxx
