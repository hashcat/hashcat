...
DECLSPEC bool pcfg_hash (PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  sha1_hmac_init_swap (&ctx0, w, len);

  sha1_hmac_update_global_swap (&ctx0, hc->esalt_bufs[hc->digest_pos].nr_buf, hc->esalt_bufs[hc->digest_pos].nr_len);
  ...
  sha1_hmac_update_global_swap (&ctx, hc->esalt_bufs[hc->digest_pos].msg_buf, hc->esalt_bufs[hc->digest_pos].msg_len[5]);
...
DECLSPEC bool pcfg_hash_global (PRIVATE_AS const pcfg_hash_ctx_t *hc, GLOBAL_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  sha1_hmac_init_global_swap (&ctx0, w, len);
...
#define PCFG_KERNEL_MXX m05400_mxx
#define PCFG_KERNEL_SXX m05400_sxx
