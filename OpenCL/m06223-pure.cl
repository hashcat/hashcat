...
__kernel void m06223_init (KERN_ATTR_TMPS_ESALT (tc64_tmp_t, tc_t))
{
...
__kernel void m06223_loop (KERN_ATTR_TMPS_ESALT (tc64_tmp_t, tc_t))
{
  const u64 gid = get_global_id (0);
...
__kernel void m06223_comp (KERN_ATTR_TMPS_ESALT (tc64_tmp_t, tc_t))
{
  const u64 gid = get_global_id (0);
  const u64 lid = get_local_id (0);
  const u64 lsz = get_local_size (0);
