...
#include "inc_cipher_cast.h"

/* CAST S-boxes */
CONSTANT_VK u32 S[8][256] =
{
	{
...
DECLSPEC void Cast5Encrypt (PRIVATE_AS const u8 *inBlock, PRIVATE_AS u8 *outBlock, PRIVATE_AS CAST_KEY *key, SHM_TYPE u32 (*s_S)[256])
{
	u32 l; GET_UINT32BE(l, inBlock, 0);
	u32 r; GET_UINT32BE(r, inBlock, 4);
	PRIVATE_AS u32 *K = key->K;
	u32 t;
...
DECLSPEC void Cast5Decrypt (PRIVATE_AS const u8 *inBlock, PRIVATE_AS u8 *outBlock, PRIVATE_AS CAST_KEY *key, SHM_TYPE u32 (*s_S)[256])
{
	u32 l; GET_UINT32BE(l, inBlock, 0);
	u32 r; GET_UINT32BE(r, inBlock, 4);
	PRIVATE_AS u32 *K = key->K;
...
DECLSPEC void Cast5SetKey (PRIVATE_AS CAST_KEY *key, u32 keylength, PRIVATE_AS const u8 *userKey, SHM_TYPE u32 (*s_S)[256])
{
	u32 i;
	PRIVATE_AS u32 *K = key->K;
	u32 X[4], Z[4];
