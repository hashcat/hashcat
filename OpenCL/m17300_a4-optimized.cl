...
DECLSPEC bool pcfg_sha3_224 (PRIVATE_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  u32 t[8];

  for (u32 i = 0; i < 8; i++) t[i] = w[i];

  pcfg_put_byte (t, len, 0x06);

  u64 a00 = hl32_to_64_S (t[1], t[0]);
  u64 a01 = hl32_to_64_S (t[3], t[2]);
  u64 a02 = hl32_to_64_S (t[5], t[4]);
  u64 a03 = hl32_to_64_S (t[7], t[6]);
  u64 a04 = 0;
  u64 a10 = 0;
  u64 a11 = 0;
  u64 a12 = 0;
  ...
  for (int round = 0; round < KECCAK_ROUNDS - 1; round++)
  {
    ...
    t64 = bc4 ^ hc_rotl64_S (bc1, 1); a00 ^= t64; a10 ^= t64; a20 ^= t64; a30 ^= t64; a40 ^= t64;
    t64 = bc0 ^ hc_rotl64_S (bc2, 1); a01 ^= t64; a11 ^= t64; a21 ^= t64; a31 ^= t64; a41 ^= t64;
    t64 = bc1 ^ hc_rotl64_S (bc3, 1); a02 ^= t64; a12 ^= t64; a22 ^= t64; a32 ^= t64; a42 ^= t64;
    t64 = bc2 ^ hc_rotl64_S (bc4, 1); a03 ^= t64; a13 ^= t64; a23 ^= t64; a33 ^= t64; a43 ^= t64;
    t64 = bc3 ^ hc_rotl64_S (bc0, 1); a04 ^= t64; a14 ^= t64; a24 ^= t64; a34 ^= t64; a44 ^= t64;
  ...
  t64 = bc4 ^ hc_rotl64_S (bc1, 1); a00 ^= t64; a10 ^= t64; a20 ^= t64; a30 ^= t64;
  t64 = bc0 ^ hc_rotl64_S (bc2, 1);                         a21 ^= t64; a31 ^= t64; a41 ^= t64;
  t64 = bc1 ^ hc_rotl64_S (bc3, 1); a02 ^= t64; a12 ^= t64; a22 ^= t64; a32 ^= t64;
  t64 = bc2 ^ hc_rotl64_S (bc4, 1); a03 ^= t64; a13 ^= t64; a23 ^= t64; a33 ^= t64; a43 ^= t64;
  t64 = bc3 ^ hc_rotl64_S (bc0, 1); a04 ^= t64;                         a34 ^= t64; a44 ^= t64;
...
DECLSPEC bool pcfg_hash (MAYBE_UNUSED PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_sha3_224 (w, len, dgst);
}
...
DECLSPEC bool pcfg_hash_global (MAYBE_UNUSED PRIVATE_AS const pcfg_hash_ctx_t *hc, GLOBAL_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_sha3_224 (t, len, dgst);
}

#define PCFG_KERNEL_MXX m17300_mxx
#define PCFG_KERNEL_SXX m17300_sxx
