...
DECLSPEC bool pcfg_dahua (PRIVATE_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  u32 t[8];
  ...
  const u32 a0 = (((h[0] >>  0) & 0xff) + ((h[0] >>  8) & 0xff)) % 62;
  const u32 a1 = (((h[0] >> 16) & 0xff) + ((h[0] >> 24) & 0xff)) % 62;
  const u32 b0 = (((h[1] >>  0) & 0xff) + ((h[1] >>  8) & 0xff)) % 62;
  const u32 b1 = (((h[1] >> 16) & 0xff) + ((h[1] >> 24) & 0xff)) % 62;
  const u32 c0 = (((h[2] >>  0) & 0xff) + ((h[2] >>  8) & 0xff)) % 62;
  const u32 c1 = (((h[2] >> 16) & 0xff) + ((h[2] >> 24) & 0xff)) % 62;
  const u32 d0 = (((h[3] >>  0) & 0xff) + ((h[3] >>  8) & 0xff)) % 62;
  const u32 d1 = (((h[3] >> 16) & 0xff) + ((h[3] >> 24) & 0xff)) % 62;
...
DECLSPEC bool pcfg_hash_global (MAYBE_UNUSED PRIVATE_AS const pcfg_hash_ctx_t *hc, GLOBAL_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  for (u32 i = 0; i < len; i++) pcfg_put_byte (t, i, (w[i / 4] >> ((i & 3) * 8)) & 0xff);

  return pcfg_dahua (t, len, dgst);
}

#define PCFG_KERNEL_MXX m24900_mxx
#define PCFG_KERNEL_SXX m24900_sxx
