...
KERNEL_FQ void HC_ATTR_SEQ m18600_init (KERN_ATTR_TMPS_ESALT (odf11_tmp_t, odf11_t))
{
  /**
   * base
   */
...
KERNEL_FQ void HC_ATTR_SEQ m18600_loop (KERN_ATTR_TMPS_ESALT (odf11_tmp_t, odf11_t))
{
  const u64 gid = get_global_id (0);
...
KERNEL_FQ void HC_ATTR_SEQ FIXED_THREAD_COUNT(FIXED_LOCAL_SIZE_COMP) m18600_comp (KERN_ATTR_TMPS_ESALT (odf11_tmp_t, odf11_t))
{
  const u64 gid = get_global_id (0);
  const u64 lid = get_local_id (0);
