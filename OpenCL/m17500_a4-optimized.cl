...
DECLSPEC bool pcfg_sha3_384 (PRIVATE_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  u32 wc[8];

  for (u32 i = 0; i < 8; i++) wc[i] = w[i];

  wc[len / 4] |= 0x06 << ((len & 3) * 8);

  /**
   * Keccak
   */

  u64 a00 = hl32_to_64_S (wc[1], wc[0]);
  u64 a01 = hl32_to_64_S (wc[3], wc[2]);
  u64 a02 = hl32_to_64_S (wc[5], wc[4]);
  u64 a03 = hl32_to_64_S (wc[7], wc[6]);
  u64 a04 = 0;
  u64 a10 = 0;
  u64 a11 = 0;
  u64 a12 = 0;
...
DECLSPEC bool pcfg_hash (MAYBE_UNUSED PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_sha3_384 (w, len, dgst);
}
...
DECLSPEC bool pcfg_hash_global (MAYBE_UNUSED PRIVATE_AS const pcfg_hash_ctx_t *hc, GLOBAL_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_sha3_384 (t, len, dgst);
}

#define PCFG_KERNEL_MXX m17500_mxx
#define PCFG_KERNEL_SXX m17500_sxx
