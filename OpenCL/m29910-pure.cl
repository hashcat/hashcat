...
KERNEL_FQ void HC_ATTR_SEQ m29910_init (KERN_ATTR_TMPS_ESALT (encdatavault_tmp_t, encdatavault_t))
{
  /**
   * base
   */
...
KERNEL_FQ void HC_ATTR_SEQ m29910_loop (KERN_ATTR_TMPS_ESALT (encdatavault_tmp_t, encdatavault_t))
{
  const u64 gid = get_global_id (0);
...
KERNEL_FQ void HC_ATTR_SEQ m29910_comp (KERN_ATTR_TMPS_ESALT (encdatavault_tmp_t, encdatavault_t))
{
  const u64 gid = get_global_id (0);
  ...
  for (int i = 1, j = 4; i < esalt_bufs[DIGESTS_OFFSET_HOST].nb_keys; i += 1, j += 4) // +4 is not a bug, 8/16 bytes are just discarded
  {
