...
#define PCFG_KERN_ATTR      KERN_ATTR_PCFG_ESALT (md5_triple_salt_t)
...
DECLSPEC void pcfg_hash_init (PRIVATE_AS pcfg_hash_ctx_t *hc, GLOBAL_AS const salt_t *salt_bufs, const u32 salt_pos, GLOBAL_AS const md5_triple_salt_t *esalt_bufs, MAYBE_UNUSED GLOBAL_AS const digest_t *digests_buf, const u32 digest_pos)
{
  ...
  hc->salt3_len = esalt_bufs[digest_pos].salt3_len;

  for (u32 i = 0; i < 64; i++) hc->salt3_buf[i] = 0;

  for (u32 i = 0, idx = 0; i < hc->salt3_len; i += 4, idx += 1)
  {
    hc->salt3_buf[idx] = esalt_bufs[digest_pos].salt3_buf[idx];
  }
}
...
DECLSPEC bool pcfg_hash (PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  md5_update (&ctx, hc->salt1_buf, hc->salt1_len);
  ...
  md5_update (&ctx, hc->salt3_buf, hc->salt3_len);
...
#define PCFG_KERNEL_MXX m32300_mxx
#define PCFG_KERNEL_SXX m32300_sxx
