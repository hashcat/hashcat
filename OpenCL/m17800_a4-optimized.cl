...
DECLSPEC bool pcfg_keccak_256 (PRIVATE_AS const u32 *w_in, const u32 len, PRIVATE_AS u32 *dgst)
{
  u32 w[8];

  for (u32 i = 0; i < 8; i++) w[i] = w_in[i];

  w[len / 4] |= 0x01u << ((len & 3) * 8);

  /**
   * Keccak
   */

  u64 a00 = hl32_to_64_S (w[1], w[0]);
  u64 a01 = hl32_to_64_S (w[3], w[2]);
  u64 a02 = hl32_to_64_S (w[5], w[4]);
  u64 a03 = hl32_to_64_S (w[7], w[6]);
  u64 a04 = 0;
  u64 a10 = 0;
  u64 a11 = 0;
  u64 a12 = 0;
...
DECLSPEC bool pcfg_hash (MAYBE_UNUSED PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_keccak_256 (w, len, dgst);
}
...
DECLSPEC bool pcfg_hash_global (MAYBE_UNUSED PRIVATE_AS const pcfg_hash_ctx_t *hc, GLOBAL_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_keccak_256 (t, len, dgst);
}

#define PCFG_KERNEL_MXX m17800_mxx
#define PCFG_KERNEL_SXX m17800_sxx
