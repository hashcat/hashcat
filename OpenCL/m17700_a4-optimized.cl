...
DECLSPEC bool pcfg_keccak224 (PRIVATE_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  u32 w0[4];
  u32 w1[4];
  ...
  // PT_ADD01 is host work for a wordlist attack, and here the candidate is built on the device.
...
DECLSPEC bool pcfg_hash (MAYBE_UNUSED PRIVATE_AS const pcfg_hash_ctx_t *hc, PRIVATE_AS u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_keccak224 (w, len, dgst);
}
...
DECLSPEC bool pcfg_hash_global (MAYBE_UNUSED PRIVATE_AS const pcfg_hash_ctx_t *hc, GLOBAL_AS const u32 *w, const u32 len, PRIVATE_AS u32 *dgst)
{
  ...
  return pcfg_keccak224 (t, len, dgst);
}

#define PCFG_KERNEL_MXX m17700_mxx
#define PCFG_KERNEL_SXX m17700_sxx
