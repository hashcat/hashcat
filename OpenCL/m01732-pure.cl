...
KERNEL_FQ KERNEL_FA void m01732_init (KERN_ATTR_TMPS_ESALT (pbkdf2_sha512_tmp_t, pbkdf2_sha512_t))
{
  ...
  /*
   * SQL Server 2025 hashes passwords as PBKDF2-HMAC-SHA512 where the
   * password byte stream fed to HMAC is the UTF-16LE representation of
   * the plain-text password.  sha512_hmac_init_global_utf16le_swap reads
   * the raw password from global memory, widens each byte to a UTF-16LE
   * code unit (byte + 0x00), and prepares the HMAC ipad/opad state.
   */
