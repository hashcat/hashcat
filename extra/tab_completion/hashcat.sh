...
_hashcat ()
{
  ...
  case "${prev}" in
    ...
    --cache-path|--induction-dir|--outfile-check-dir) # these three take a directory
    ...
    --keyboard-layout-mapping)
      _hashcat_files_include "${cur}" '.*\.\(table\|hckmap\)$'
      COMPREPLY=($(compgen -W "${hashcat_file_list}" -- ${hashcat_select}))
      return 0
      ;;
  ...
  case "${no_opts}" in
    ...
    *)
      case "${attack_mode}" in
        ...
        5)
          # a hash file, a wordlist, then as many tables as the user cares to name, so this one never
          # stops offering files
